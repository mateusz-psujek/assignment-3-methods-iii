...
check[[1]] %>% 
  ...
  ungroup %>%
  mutate(true_mean = c(informed_pop_effects, - informed_pop_effects))
...
### Creating the models
```{r}
prior_b <- normal(location = 0, scale = 0.5, autoscale = T)
prior_intercept <- normal(0, 1, autoscale = T)
#setting the autoscale to TRUE makes the model adjust the scale of the prior to the standard deviation of each parameter by multiplying the scale argument by 1 /sd(given variable)
...
### Creating the models
```{r}
prior_b <- normal(location = 0, scale = 0.3, autoscale = T)
prior_intercept <- normal(0, 1, autoscale = T)
